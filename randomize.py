...
def getRandomArrays(inputArray: np.ndarray, count: int, percent=80, seed=42, noData=-9999) -> tuple:
    ...
    lsCount = len(values)
    lsCountInTrainArray = int(round(lsCount/100*percent)) # without round int() rounds down.
    ...
    for i in range(count):
        ...
        valValues = np.setdiff1d(values, trainValues) # Val = Values in values but not in train
        trainArray = inputArray.copy()
        np.put(trainArray, valValues, [0]) # Replaces all valValues with 0 for TrainArray in place.
        valArray = inputArray.copy()
        np.put(valArray, trainValues, [0]) # Replaces all trainValues with 0 for ValArray in place.
...
if __name__ == "__main__":
    import timeit
    x = timeit.timeit("getRandomArray(np.arange(815262).reshape(826, 987), 100, noData=899)", globals=globals(), number=1)
    print(x)